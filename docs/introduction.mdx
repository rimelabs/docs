---
title: Introduction
description: "An overview of Rime's speech platform, product capabilities, and developer interfaces."
---

Rime is a speech platform for turning text into natural, responsive audio. It powers spoken content, conversational agents, telephony, accessibility features, games, and any product that needs a voice.

The [Rime dashboard](https://app.rime.ai/) provides a visual workspace for exploring voices and generating speech. The same speech capabilities are available to applications and developer tools through Rime's APIs.

## What you can do with Rime

<Columns cols={2}>
  <Card title="Generate speech" icon="waveform" href="/docs/quickstart-five-minute">
    Turn text into playable or downloadable audio in formats suited to web, mobile, telephony, and media workflows.
  </Card>

  <Card title="Stream speech in real time" icon="signal-stream" href="/docs/streaming">
    Play audio as it is generated, or send text incrementally for low-latency conversations and voice agents.
  </Card>

  <Card title="Choose voices, models, and languages" icon="microphone-lines" href="/docs/voices">
    Browse distinct speakers and select the speech model, language, and sound that fit your application.
  </Card>

  <Card title="Shape how speech sounds" icon="sliders" href="/docs/prompting">
    Guide delivery, pacing, spelling, pauses, pronunciation, and the way numbers, dates, and other text are spoken.
  </Card>

  <Card title="Build voice agents" icon="messages" href="/docs/quickstart-livekit">
    Use Rime as the speech layer in conversational AI, customer support, telephony, and interactive assistants.
  </Card>

  <Card title="Connect voice platforms" icon="puzzle-piece" href="/docs/livekit">
    Integrate with LiveKit, Pipecat, Vapi, Daily, SignalWire, VideoSDK, and other voice application frameworks.
  </Card>

  <Card title="Create a custom voice" icon="clone" href="/platform/voice-cloning">
    Make a branded enterprise voice available to the same speech API as Rime's voice catalog.
  </Card>

  <Card title="Run Rime in your environment" icon="server" href="/docs/on-prem/quickstart">
    Use Rime's regional cloud endpoints or deploy supported models on your own infrastructure.
  </Card>
</Columns>

## Work with Rime

<Columns cols={2}>
  <Card title="Dashboard" icon="browser" href="https://app.rime.ai/">
    Audition voices, generate audio, inspect normalized text, create API tokens, manage teams, and review billing.
  </Card>

  <Card title="HTTP and WebSocket APIs" icon="brackets-curly" href="/docs/api-reference">
    Generate complete audio responses, stream audio, receive word timestamps, list voices, and use Rime's language tools.
  </Card>

  <Card title="Application code" icon="code" href="/docs/quickstart-five-minute">
    Use standard HTTP and WebSocket clients from Python, JavaScript, Go, or any server-side language; no Rime-specific SDK is required.
  </Card>

  <Card title="CLI" icon="terminal" href="/docs/quickstart-cli">
    Log in, generate, play, save, and inspect speech from a terminal or automation script.
  </Card>

  <Card title="MCP server" icon="robot" href="/docs/mcp">
    Let Claude, Codex, and compatible tools browse voices, generate samples, inspect pronunciation and text normalization, and scaffold integrations.
  </Card>

  <Card title="Application integrations" icon="plug" href="/docs/pipecat">
    Add Rime to existing voice-agent frameworks, web applications, communication platforms, and AI development tools.
  </Card>
</Columns>

One Rime API token authenticates the API, CLI, MCP tools, and supported integrations. Tokens, team access, and billing are managed in the [Rime dashboard](https://app.rime.ai/).
