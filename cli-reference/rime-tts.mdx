...
<Note>The `mist` and `mistv2` models require `--format mp3`. The CLI returns an error if you omit it. `mistv3` defaults to WAV and does not require `--format mp3`.</Note>
