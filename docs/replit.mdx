...
This guide demonstrates how to add Rime's text-to-speech (TTS) to apps built with [Replit](https://replit.com/), an AI-powered platform that generates code from users' text descriptions. Rime is a TTS API that generates natural-sounding voice audio. When your Replit app needs to speak, it sends text to Rime's API and receives audio back.
...
The Replit agent generates code that reads the `RIME_API_KEY` from your **Secrets** and uses it to authenticate with Rime's API.

## Add your API key

Once Replit has created your app, add your Rime API key as a Secret:

1. Press **Cmd + K** (Mac) or **Ctrl + K** (Windows and Linux) to open the command palette.
