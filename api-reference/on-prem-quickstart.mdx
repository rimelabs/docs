...
You'll need to permit outbound network traffic to `http://optimize.rime.ai/usage` and `http://optimize.rime.ai/license` to verify with our servers that you have an active on-prem licensing agreement and to register usage. Additionally, you'll need access to [quay.io](https://hub.docker.com/), a container image repository platform, so you'll need to allow outbound traffic to their servers on port `443`.
