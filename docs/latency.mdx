...
<Tip>For the lowest cloud latency, use `mistv3` — typical TTFB is well below 100ms. `mistv2` and `mist` are also significantly faster than the deprecated v1 models.</Tip>
