---
title: 'Arcana WebSockets (deprecated)'
description: "Deprecated Arcana plain-text WebSocket reference. Arcana is retired on August 15, 2026 at 12:00 UTC; migrate cloud integrations to Coda."
...
<Warning>
**Arcana is retired on August 15, 2026 at 12:00 UTC.** Do not start a new Arcana integration. After the cutoff, cloud requests that specify an Arcana model are served by Coda. Follow the [Arcana migration guide](/docs/arcana-sunset) before then so you can test the change.

Until the cutoff, include `modelId=arcana` in the connection query. If you omit it, the server routes the request to Mist v3 and speakers outside the Mist v3 catalog can fail with `Speaker not found`.
</Warning>
...
class RimeClient:
    def __init__(self, speaker, api_key):
        self.url = f"wss://users-ws.rime.ai/ws?speaker={speaker}&modelId=arcana&audioFormat=wav"
        self.auth_headers = {
