...
<ParamField body="modelId" type="string">
  Choose `mistv3` for Rime's fastest model with sub-100ms TTFB, `mistv2` for the previous fast model, or `mist` for the legacy model (default: `mist`)
</ParamField>

<ParamField body="pauseBetweenBrackets" type="bool" default="false">
  **mist/mistv2 only.** When set to true, adds pauses between words enclosed in angle brackets. The number inside the brackets specifies the pause duration in milliseconds. <br/>
...
<ParamField body="phonemizeBetweenBrackets" type="bool" default="false">
  **mist/mistv2 only.** When set to true, you can specify the phonemes for a word enclosed in curly brackets. <br/>
...
<ParamField body="speedAlpha" type="float" default="1.0">
  Adjusts the speed of speech. Behavior depends on model: for `mist` and `mistv2`, lower than 1.0 is faster and higher than 1.0 is slower. For `mistv3` and `arcana`, higher than 1.0 is faster and lower than 1.0 is slower.
</ParamField>
