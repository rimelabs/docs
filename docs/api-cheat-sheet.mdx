...
Set `modelId` explicitly on `/ws3` — without it, synthesis defaults to `mistv2`. Message schemas: [Coda WebSocket reference](/api-reference/coda/websockets-json) · Buffering control: [Segmentation](/docs/websockets-segment) · Web-app bridge pattern: [Next.js voice-agent guide](/docs/voice-agent-nextjs#3-optional-stream-over-websockets)
...
| `modelId` | `coda` (flagship), `arcana`, `mistv3`, `mistv2`, `mist` (legacy) | Set it explicitly — WebSocket endpoints default to `mistv2`. |
