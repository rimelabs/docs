...
description: "Rime TTS latency benchmarks: sub-200ms end-to-end via the cloud API, sub-100ms on-prem with Coda, 37ms TTFA with Mist v3 — plus what affects response time and how to reduce it."
...
- Request audio that meets the needs of your application. Remember: Smaller payloads move across the network more quickly. For example, if you're building an application for telephony, request 8 kHz (8,000 Hz) audio using the `samplingRate` parameter.
