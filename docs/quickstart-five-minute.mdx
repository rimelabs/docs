...
<Info>
  No SDK is required (or available): there is no official Rime package on npm or PyPI. Every script below uses only the standard library or built-in `fetch`. Building inside a web framework? See the voice-agent guides for [Next.js](/docs/voice-agent-nextjs), [Vite](/docs/voice-agent-vite), and [FastAPI](/docs/voice-agent-fastapi).
</Info>
