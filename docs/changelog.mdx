...
* Arcana release `20260223`:
  * v3 (multilingual)
    * Supported languages: `de`, `en`, `es`, `fr`, `he`, `hi`, `ja`, `pt`, `si`, `ta`.
  * General performance improvements and bug fixes.
</Update>
...
* Arcana release `20260204` (Arcana v3):
  * Added support for languages: English (`lang=eng`), French (`lang=fra`), German (`lang=ger`), Hebrew (`lang=heb`), Hindi (`lang=hin`), Japanese (`lang=jpn`), Portuguese (`lang=por`), Spanish (`lang=spa`).
  * Languages can be specified using either 2-character codes (`en`, `fr`, `de`, `he`, `hi`, `ja`, `pt`, `es`) or 3-character codes (`eng`, `fra`, `ger`, `heb`, `hin`, `jpn`, `por`, `spa`).
</Update>
