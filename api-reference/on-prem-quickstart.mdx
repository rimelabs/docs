...
You should be getting a time-to-first-frame of ~400ms on H100.
...
**Other Prerequisites**:
- GPU requirements:
  - For Mist: NVIDIA T4, L4, A10G, or any equivalent or higher-performing GPU
  - For Arcana: NVIDIA H100, or any equivalent or higher-performing GPU
...
### Artifact Registry Image Pulling

**Text-to-Speech (TTS) Image**: Pull the TTS service image.
...
**API Image**: Similarly, pull the latest API service image to be used in conjunction with the TTS service.

```bash Pull API Image
docker pull us-docker.pkg.dev/rime-labs/api/service:20250731
```
