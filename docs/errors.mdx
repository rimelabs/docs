...
description: "What the Rime API returns when a request fails, per transport, and what to capture when you need support."
...
<Warning>**A retried synthesis is billed again.** Rime bills by characters synthesized, and there is no request ID or idempotency key, so Rime cannot detect that two identical requests were meant to be one. If a WebSocket closes partway through a long utterance, restarting it re-bills the whole utterance. Decide deliberately whether to resume from a sentence boundary in your own application or accept the duplicate cost.</Warning>
