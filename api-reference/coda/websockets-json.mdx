...
<ParamField body="modelId" type="string">
  This value **must** be set to `coda` — without it, the server routes to the Mist v3 backend, and speakers outside the Mist v3 catalog fail with a "Speaker not found" error.
</ParamField>
