...
Install [Python 3.10 or later](https://www.python.org/downloads/). Verify your installation by running the following command in your terminal:
...
This creates the WebRTC transport and enables audio I/O as well as Silero VAD for detecting when the user starts and stops speaking.
...
These configure OpenAI for STT and LLM responses, and the Rime `arcana` model for TTS.
...
- **Transcription:** converts speech to text via the STT provider
- **User context:** aggregates the user's message into the conversation history
- **LLM response:** generates a reply based on the conversation so far
- **Speech synthesis:** converts the LLM's text response to audio via Rime TTS
- **Audio out:** streams the synthesized speech back to the user
- **Assistant context:** records the assistant's response in the conversation history

The context aggregator appears twice to capture both sides of the conversation.

Finally, add the task runner and an event handler for greeting the user:
...
When you run the agent, Pipecat starts a local HTTP server. Open the browser client to connect via WebRTC. The server runs locally, but API calls are made to OpenAI's and Rime's cloud services.
