...
<Note>
  This endpoint is superseded by [`POST /speech-qa/submissions`](/api-reference/speech-qa/submissions), which performs the same coverage analysis, persists the result as a submission, and accumulates out-of-vocabulary words against your billable for review. New integrations should use the Speech QA API. This endpoint will continue to work for now.
</Note>
...
We can add new words to our dictionary as needed within around 24 hours. Or you can immediately [generate a custom pronunciation](/docs/custom-pronunciation), which uses the [Rime phonetic alphabet](/docs/custom-pronunciation#the-rime-phonetic-alphabet).
