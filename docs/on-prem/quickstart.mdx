...
#### JSON websockets
The JSON WebSocket endpoint compatible with both `arcana` models as well as `mist` will be served at port `8003`. For example, `ws://localhost:8003`, which is equivalent to our [cloud JSON WebSocket API](/docs/websockets).
See the [Arcana JSON WebSocket docs](/api-reference/arcana/websockets-json) and the [Mist JSON WebSocket docs](/api-reference/mistv2/websockets-json) depending
