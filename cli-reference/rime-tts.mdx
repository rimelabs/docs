...
Synthesize text to speech in WAV or MP3. The CLI picks the format from the model: `mist` and `mistv2` output MP3, while `coda`, `mistv3`, `arcana`, and `arcanav2` default to WAV. Use `--format` to override.
