...
Rime provides regional API endpoints so you can route requests to the data center closest to your application's deployment:

| Endpoint | Region |
| -------- | ------ |
| `https://users.rime.ai/v1/rime-tts` | US West (us-west-1) — default |
| `https://users-east.rime.ai/v1/rime-tts` | US East (us-east-1) |
| `https://users-west.rime.ai/v1/rime-tts` | US West (us-west-1) |

WebSocket equivalents follow the same pattern: `wss://users-east-ws.rime.ai/ws` and `wss://users-west-ws.rime.ai/ws`.

Choosing the endpoint closest to your deployment typically reduces round-trip time compared to using the default URL from a distant region. Use the [`rime speedtest`](/cli-reference/rime-monitoring) command to measure TTFB against each endpoint from your machine.
