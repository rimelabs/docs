...
* Regoinal endpoints available:
  * `wss://users-east-ws.rime.ai/` offers all the websockets apis currently served served via `wss://users-ws.rime.ai/ws*` and may provide better latency
  for clients closer to `us-east-1`.
  * `https://users-east.rime.ai` offers all the apis currently served via `https://users.rime.ai`, and may provide better latency
  for clients closer to `us-east-1`.
  * Additionally `https://users-west.rime.ai` offers all the apis currently served via `https://users.rime.ai`, and may provide better latency
...
* Arcana release `20260204` (Arcana v3):
  * Added support for 11 languages: Arabic (`lang=ara`), English (`lang=eng`), French (`lang=fra`), German (`lang=ger`), Hebrew (`lang=heb`), Hindi (`lang=hin`), Japanese (`lang=jpn`), Portuguese (`lang=por`), Sinhala (`lang=sin`), Spanish (`lang=spa`), and Tamil (`lang=tam`).
  * Languages can be specified using either 2-character codes (`ar`, `en`, `fr`, `de`, `he`, `hi`, `ja`, `pt`, `si`, `es`, `ta`) or 3-character codes (`ara`, `eng`, `fra`, `ger`, `heb`, `hin`, `jpn`, `por`, `sin`, `spa`, `tam`).
</Update>
