...
### Binary Websockets Endpoints

The binary (non-json) websockets endpoint will be served at port `8002`.
For example `ws://localhost:8002` which will be equivalent to our [cloud websockets API](/api-reference/endpoint/websockets).

### Json Websockets Endpoints

The json websockets endpoint will be served at port `8001`.
