...
## tldr: Rime is really fast. 
Sub-250ms latency is standard; [contact us](mailto:info@rime.ai) to optimize further.
...
curl -X POST http://users.rime.ai/v1/rime-tts \
  ...
  -d '{"text": "I love the book that she gave me. And I was so happy that she gave it to me.", "speaker": "juan",  "modelId": "mist"}'
```

### Recommendations for reducing response time

<Tip>Rime's `mist` models are across the board faster than our `v1` models. Using the `modelId` parameter, you can select our Mist model for TTS inference.</Tip>
