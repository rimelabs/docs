...
On-prem keeps all audio and text within your own network; nothing is sent to Rime's cloud. That supports data-residency requirements and compliance with data privacy and protection regulations.
...
#### JSON websockets
The API service serves the JSON WebSocket endpoint for `coda`, `arcana`, and `mist` models on port `8003`. For example, `ws://localhost:8003`, which is equivalent to our [cloud JSON WebSocket API](/docs/websockets).
...
#### Non-JSON websockets
The API service serves the non-JSON WebSocket endpoint on port `8002`. For example, `ws://localhost:8002`, which is equivalent to our [cloud WebSocket API](/docs/websockets).
