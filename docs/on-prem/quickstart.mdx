...
- NVIDIA drivers
  - Minimum: R545 (`545.23.08` or higher)
  - Recommended: R580 (`580.95.05` or higher)
- Docker
...
R545 is the minimum supported driver branch for Rime on-prem containers. For production deployments, use R580 or a newer NVIDIA-supported branch.
NVIDIA marks older data center driver branches that are not listed in its [supported drivers and CUDA toolkit versions](https://docs.nvidia.com/datacenter/tesla/drivers/595/supported-drivers-and-cuda-toolkit-versions.html) matrix as end of life.
...
Set `RIME_CUDA_COMPAT_MODE` on each model container to choose how the image loads NVIDIA driver libraries:

- `native` (default): use the driver libraries provided by the host through NVIDIA Container Toolkit.
- `compat`: prepend the CUDA compatibility libraries included in the image before starting the model.
  Use this when the host driver is older than the CUDA user-mode libraries in the image but is still covered by NVIDIA's [CUDA forward compatibility](https://docs.nvidia.com/deploy/cuda-compatibility/forward-compatibility.html) support matrix.
