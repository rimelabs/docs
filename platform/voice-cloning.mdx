...
Rime offers high-quality Enterprise Voice Cloning (EVC) for customers on our Growth and Enterprise plans.

<Steps>
  <Step title="Prepare data">
    High-quality voice cloning data comes from clean, consistent, and varied recordings. Use a low-reverb space with only the target speaker. Around 30 to 60 minutes is enough for basic cloning, while two to five hours or more yields highly natural results. Keep the same microphone, distance, and speaking energy throughout sessions. Use a pop filter, include natural breaths and pauses, and record over multiple days to capture subtle variations while avoiding fatigue.
    ...
    Ideally the script **should contain the following elements**:
        ...
        - Alphanumeric sequences like phone numbers, addresses, etc.
        - The performance and prosody that you're looking for (e.g. outbound calling, inbound customer support, etc.)
  ...
  <Step title="Delivery criteria">
    Deliver the audio in the following format:
        - **Channels**: Mono
        - **Format**: Lossloss format (.wav or .flac)
        - **Sampling rate**: At least 44.1kHz
        - **Bit depth**: At least 16-bit
  </Step>
  <Step title="Delivery and completion">
    Deliver the finished audio to the Rime team, and inform us which foundational model you want the clone to be trained on, e.g. Mist, Arcana, or both.
    ...
    curl --request POST \
        ...
        --data '{
        "speaker": "84462a19-dc31-4f30-91e5-19bfc6415a8",
        "text": "<string>",
        "modelId": "arcana"
    }'
    ```
