...
<Warning>**A retried synthesis is billed again.** Rime bills by characters synthesized and there is no request ID or idempotency key, so two identical requests cannot be recognized as one. Restarting a long utterance after a mid-stream failure re-bills the whole utterance, including the part you already received. Decide deliberately whether to resume from a sentence boundary in your own application or accept the duplicate cost. Rime provides no resume or deduplication mechanism.</Warning>
