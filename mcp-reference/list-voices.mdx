...
List Rime's voice catalog as JSON keyed by model and language: `{ modelId: { langCode: [speaker, ...] } }`. Optionally filter by model and/or language. Use [`get_voice_details`](/mcp-reference/get-voice-details) for demographics and the exact `lang` value to pass to [`synthesize_speech`](/mcp-reference/synthesize-speech).
