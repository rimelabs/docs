...
- **Arcana v3** — our flagship TTS model that combines ultra-realistic, expressive voices (including laughter and breathing) with **low latency (~120 ms TTFB)** and **native multilingual code-switching** across more than 10 languages. It also supports enterprise-grade ergonomics for high-volume, real-time deployments at scale.
