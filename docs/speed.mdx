...
<Note>
Coda and Mist v3 also accept `speedAlpha`, but it works in the **opposite direction** from `timeScaleFactor` (higher than 1.0 is faster, lower is slower). Going forward we recommend `timeScaleFactor` for these models; it's the parameter we'll continue to support across future flagship models.
</Note>
