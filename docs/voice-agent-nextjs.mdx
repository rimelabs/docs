...
wss.on("connection", (client) => {
  ...
  client.on("message", (data) => {
    if (rime.readyState === WebSocket.OPEN) rime.send(data.toString());
  });
...
ws.onmessage = async (e) => {
  ...
  if (msg.type === "chunk") {
    // base64 MP3 chunk → queue it for playback
    const bytes = Uint8Array.from(atob(msg.data), (c) => c.charCodeAt(0));
    enqueue(new Blob([bytes], { type: "audio/mpeg" }));
  }
  if (msg.type === "done") console.log("utterance complete");
};
...
// Interrupt (barge-in): drop whatever is still buffered
function interrupt() {
