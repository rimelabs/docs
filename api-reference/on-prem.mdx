...
![On-Premise Components](/images/on-prem-components.png)
...
You'll need to permit outbound network traffic to `<address-will-be-added>` to verify with our servers that you have an active on-prem licensing agreement and to register usage. Additionally, you'll need access to [DockerHub](https://hub.docker.com/), a container image repository platform, so you'll need to allow outbound traffic to their servers on port `443`.
