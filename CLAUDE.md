...
- Address the reader as "you." Name the API, model, CLI, server, browser, or dashboard when it performs an action. Use "Rime" for company-level facts and product commitments. Avoid "we," "our," and "us" in ordinary instructions and capability claims; reserve them for communication from actual people. Do not replace every first-person pronoun with "Rime" when a more specific system actor is clearer.
- In ordered interface instructions, lead with the location and end with the action: "In the Rime dashboard, open **API Tokens**." Prefer "select" or "open" to device-specific words such as "click," and link directly to the relevant dashboard page when one is available.
- Introduce every code sample with its result, side effect, or relevant constraint. Write a specific sentence such as "This request streams a WAV file to `output.wav`," not boilerplate such as "The following example demonstrates."
- Keep content types distinct. A quickstart should teach the recommended path and define success. A conceptual guide should explain decisions, tradeoffs, and consequential edge cases. A reference page should document the contract without repeating the tutorial.
...
- In integration guides, place upstream defaults and failure modes beside the setting that triggers them. Do not make the reader finish the happy path before learning that a partner default is incompatible or deprecated.
