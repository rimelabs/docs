...
Rime currently has five models in production: `coda`, `arcanav3`, `arcanav2`, `mistv3`, and `mistv2`. All are available via the cloud API and on-premises, and all stream audio in real time over [HTTP and WebSockets](/docs/streaming).
...
<Note>The controls in this section apply to Arcana only. Coda accepts these request parameters but does not currently apply them. Mist v3 and Mist v2 do not expose them.</Note>

Arcana supports several additional controls due to its LLM backbone. We recommend leaving these on the default values.
