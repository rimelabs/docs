...
description: "Stream Rime TTS in real time over WebSockets for voice agents: choose between /ws3 (flagship JSON), /ws2 (legacy JSON), and /ws (raw binary), with word-level timestamps and interruption handling."
icon: plug
---
...
<Warning>
  The token **must** be sent in the `Authorization` header. The API key cannot be passed as a query parameter (`?api_key=…`) — a connection without the header is rejected with `401 missing headers`.

  Because the browser `WebSocket` API can't set request headers, you cannot connect to these endpoints directly from browser JavaScript. Connect from a server-side process (Node, Python, etc.), or proxy the connection through your own backend — which keeps your API key off the client anyway.
</Warning>
...
|---|---|---|---|---|
| **`/ws3`** (JSON) | [Coda](/api-reference/coda/websockets-json) | [Arcana](/api-reference/arcana/websockets-json) | [Mist v3](/api-reference/mistv3/websockets-json) | — |
| **`/ws2`** (JSON) | — | — | — | [Mist v2](/api-reference/mistv2/websockets-json) |
| **`/ws`** (binary) | [Coda](/api-reference/coda/websockets) | [Arcana](/api-reference/arcana/websockets) | [Mist v3](/api-reference/mistv3/websockets) | [Mist v2](/api-reference/mistv2/websockets) |
