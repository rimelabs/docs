---
title: 'Custom pronunciation (Mist v2 only)'
---

<Note>Custom pronunciation is supported by **Mist v2** only.</Note>
...
Custom pronunciations use Rime's phonetic alphabet (detailed [below](#the-rime-phonetic-alphabet)), which is inspired by the International Phonetic Alphabet (IPA). For example, the word `custom` would be represented as `{k1Ast0xm}`. This even works for made-up words, like `gorbulets`, which would be `{g1orby0ul2Ets}`.
...
The **Check Coverage** tool is available on the Rime dashboard: [rime.ai/dashboard/coverage](https://app.rime.ai/coverage). You can also check coverage [via the API](/api-reference/endpoint/oov).
...
The **Pronunciation** tool is available on the Rime dashboard: [rime.ai/dashboard/pronunciation](https://app.rime.ai/pronunciation).
