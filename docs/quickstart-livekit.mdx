...
For non-streaming TTS, LiveKit uses `StreamAdapter` to split streamed text into chunks before synthesis.
Smaller chunks improve latency; larger chunks can improve prosody for multi-sentence text.
...
Increase `min_token_len` to batch more text into each TTS request. For example, set it above the length
of a scripted greeting to send the greeting as one request.
