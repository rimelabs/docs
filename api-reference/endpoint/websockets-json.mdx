...
<ParamField body="lang" type="string" default="eng">
  Specifies the language spoken by the provided speaker. Must be one of: `eng` (English) or `spa` (Spanish). If provided, it must match the speaker's language.
</ParamField>
...
<ParamField body="segment" type="string" default="bySentence">
  ...
  Note: For backward compatibility, setting `immediate=true` in query params is equivalent to `segment=immediate`
</ParamField>
